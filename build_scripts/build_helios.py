...
class HeliosBuilder:
    ...
    def apply_helios_core_patches(self) -> None:
        ...
        without shipping a helios-core commit. Each patch should be idempotent and
        self-detecting: if the target text is absent (already fixed upstream or
        refactored), the patch is a no-op. When a future helios-core version
        incorporates the fix, the corresponding patch can be removed.

        Currently empty: the only patch (LiDAR.cpp OpenMP guards) is now committed
        upstream as of helios-core's synthetic-scan ``#ifdef _OPENMP`` guards, so
        the build-time monkey-patch is no longer needed.
        """
        return
