...
class Context:
    ...
    def addAdaptiveTileObject(self, center: vec3 = vec3(0, 0, 0), size: vec2 = vec2(1, 1),
                              rotation: SphericalCoord = SphericalCoord(1, 0, 0),
                              refinement: Optional[AdaptiveTileRefinement] = None,
                              color: Optional[RGBcolor] = None,
                              texturefile: Optional[str] = None,
                              texture_repeat: Optional[int2] = None) -> int:
        """
        ...
        Args:
            ...
            size: Size in x and y directions (default: 1x1)
            rotation: Spherical rotation (default: no rotation)
            refinement: Parameters controlling the adaptive sub-patch resolution
                (default: ``AdaptiveTileRefinement()``)
            color: Optional RGB color
            texturefile: Optional texture image file path
            texture_repeat: Optional texture repetitions in x and y. Unlike
                :meth:`addTileObject`, the count is applied exactly -- the base grid is snapped to
                a multiple of it rather than the count being reduced.
        ...
        Raises:
            ValueError: If parameters are invalid
            RuntimeError: If the refinement is rejected by helios, e.g. because
                ``subpatch_size_max`` exceeds half the smaller tile dimension, or because the
                requested refinement would generate more than 2,000,000 sub-patches
            NotImplementedError: If object-returning functions unavailable

        Note:
            Sub-patches are ordered by quadtree traversal, not row-major, so they cannot be
            indexed by grid position. Adaptive tiles have no subdivision count, so
            :meth:`getTileObjectSubdivisionCount` and :meth:`setTileObjectSubdivisionCount` do
            not apply to them.

        Examples:
            >>> # Ground plane resolved finely beneath a plant at the origin
            >>> refinement = AdaptiveTileRefinement(
            ...     target=vec2(0, 0), subpatch_size_min=0.02, subpatch_size_max=2.0
            ... )
            >>> obj_id = ctx.addAdaptiveTileObject(
            ...     center=vec3(0, 0, 0), size=vec2(50, 50), refinement=refinement
            ... )

            >>> # Check the cost before committing to it
            >>> ctx.predictAdaptiveTileObjectSubpatchCount(vec2(50, 50), refinement)  # doctest: +SKIP
            21374
        """
        self._check_context_available()
        ...
        if not isinstance(rotation, SphericalCoord):
            raise ValueError(f"Rotation must be a SphericalCoord, got {type(rotation).__name__}")
        if not isinstance(refinement, AdaptiveTileRefinement):
            raise ValueError(f"Refinement must be an AdaptiveTileRefinement or None, got {type(refinement).__name__}")
        if color is not None and not isinstance(color, RGBcolor):
            raise ValueError(f"Color must be an RGBcolor or None, got {type(color).__name__}")
        if texture_repeat is not None and not isinstance(texture_repeat, int2):
            raise ValueError(f"texture_repeat must be an int2 or None, got {type(texture_repeat).__name__}")
        ...
        if texture_repeat is not None:
            if texturefile is None:
                raise ValueError("texture_repeat requires texturefile")
            return context_wrapper.addAdaptiveTileObject_texture_repeat(
                self.context, center.to_list(), size.to_list(), rotation_list,
                refinement.to_list(), texturefile, texture_repeat.to_list()
            )
        elif texturefile:
            return context_wrapper.addAdaptiveTileObject_texture(
                self.context, center.to_list(), size.to_list(), rotation_list,
                refinement.to_list(), texturefile
            )
        elif color:
            return context_wrapper.addAdaptiveTileObject_color(
                self.context, center.to_list(), size.to_list(), rotation_list,
                refinement.to_list(), color.to_list()
            )
        else:
            return context_wrapper.addAdaptiveTileObject_basic(
                self.context, center.to_list(), size.to_list(), rotation_list,
                refinement.to_list()
            )

    def predictAdaptiveTileObjectSubpatchCount(self, size: vec2,
                                               refinement: Optional[AdaptiveTileRefinement] = None,
                                               texture_repeat: Optional[int2] = None) -> int:
        """
        ...
        Args:
            size: Size of the tile in the x- and y-directions
            refinement: Parameters controlling the adaptive sub-patch resolution
                (default: ``AdaptiveTileRefinement()``)
            texture_repeat: Texture repetitions the tile would use (default: 1x1)
        ...
        Raises:
            ValueError: If parameters are invalid
            RuntimeError: Raised for a refinement that would generate too many sub-patches, rather
                than reporting a count, so that this and :meth:`addAdaptiveTileObject` answer the
                question the same way

        Examples:
            >>> refinement = AdaptiveTileRefinement(subpatch_size_min=0.02, subpatch_size_max=2.0)
            >>> ctx.predictAdaptiveTileObjectSubpatchCount(vec2(50, 50), refinement)  # doctest: +SKIP
            21374
        """
        self._check_context_available()
        ...
        if texture_repeat is None:
            texture_repeat = int2(1, 1)

        if not isinstance(size, vec2):
            raise ValueError(f"Size must be a vec2, got {type(size).__name__}")
        if not isinstance(refinement, AdaptiveTileRefinement):
            raise ValueError(f"Refinement must be an AdaptiveTileRefinement or None, got {type(refinement).__name__}")
        if not isinstance(texture_repeat, int2):
            raise ValueError(f"texture_repeat must be an int2 or None, got {type(texture_repeat).__name__}")

        return context_wrapper.predictAdaptiveTileObjectSubpatchCount(
            self.context, size.to_list(), refinement.to_list(), texture_repeat.to_list()
        )
    ...
    def copyPrimitiveData(self, sourceUUID: int, destinationUUID: int) -> None:
        ...
        Example:
            ...
            >>> context.setPrimitiveDataFloat(source_uuid, "temperature", 25.5)
    ...
    def getTileObjectTextureRepeat(self, objID: int) -> int2:
        ...
        """
        self._check_context_available()
        x, y = context_wrapper.getTileObjectTextureRepeatWrapper(self.context, objID)
        return int2(x, y)

    def getTileObjectEffectiveTextureRepeat(self, objID: int) -> int2:
        ...
        """
        self._check_context_available()
        x, y = context_wrapper.getTileObjectEffectiveTextureRepeatWrapper(self.context, objID)
        return int2(x, y)

    # Adaptive Tile
    def getAdaptiveTileObjectCenter(self, objID: int) -> vec3:
        """Get the Cartesian coordinates of the center of an adaptive tile object."""
        self._check_context_available()
        x, y, z = context_wrapper.getAdaptiveTileObjectCenterWrapper(self.context, objID)
        return vec3(x, y, z)

    def getAdaptiveTileObjectSize(self, objID: int) -> vec2:
        """Get the dimensions of an entire adaptive tile object."""
        self._check_context_available()
        x, y = context_wrapper.getAdaptiveTileObjectSizeWrapper(self.context, objID)
        return vec2(x, y)

    def getAdaptiveTileObjectNormal(self, objID: int) -> vec3:
        """Get a unit vector normal to an adaptive tile object surface."""
        self._check_context_available()
        x, y, z = context_wrapper.getAdaptiveTileObjectNormalWrapper(self.context, objID)
        return vec3(x, y, z)

    def getAdaptiveTileObjectVertices(self, objID: int) -> List[vec3]:
        """Get the Cartesian coordinates of each of the four corners of an adaptive tile object."""
        self._check_context_available()
        triples = context_wrapper.getAdaptiveTileObjectVerticesWrapper(self.context, objID)
        return [vec3(x, y, z) for x, y, z in triples]

    def getAdaptiveTileObjectRefinement(self, objID: int) -> AdaptiveTileRefinement:
        """Get the refinement parameters that were requested when the object was created."""
        self._check_context_available()
        tx, ty, smin, smax, exponent = context_wrapper.getAdaptiveTileObjectRefinementWrapper(self.context, objID)
        return AdaptiveTileRefinement(target=vec2(tx, ty), subpatch_size_min=smin,
                                      subpatch_size_max=smax, transition_exponent=exponent)

    def getAdaptiveTileObjectBaseSubdivisionCount(self, objID: int) -> int2:
        ...
        """
        self._check_context_available()
        x, y = context_wrapper.getAdaptiveTileObjectBaseSubdivisionCountWrapper(self.context, objID)
        return int2(x, y)

    def getAdaptiveTileObjectMaxRefinementLevel(self, objID: int) -> int:
        """Get the maximum quadtree refinement level, i.e. the number of times a base cell may be subdivided."""
        self._check_context_available()
        return context_wrapper.getAdaptiveTileObjectMaxRefinementLevelWrapper(self.context, objID)

    def getAdaptiveTileObjectSubpatchSizeRange(self, objID: int) -> vec2:
        """Get the sub-patch edge lengths actually achieved, as opposed to those requested.

        Returns:
            A vec2 holding the achieved minimum edge length in ``x`` and the achieved maximum edge
            length in ``y``.
        """
        self._check_context_available()
        x, y = context_wrapper.getAdaptiveTileObjectSubpatchSizeRangeWrapper(self.context, objID)
        return vec2(x, y)

    def getAdaptiveTileObjectTextureRepeat(self, objID: int) -> int2:
        ...
        """
        self._check_context_available()
        x, y = context_wrapper.getAdaptiveTileObjectTextureRepeatWrapper(self.context, objID)
        return int2(x, y)
