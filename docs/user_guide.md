...
|--------|-------------|-------------------|---------|
| Tile | Patch subdivided into uniform grid of sub-patches. | <ul><li>\ref pyhelios.Context.Context::addTileObject "addTileObject(center, size, rotation, subdiv)"</li><li>\ref pyhelios.Context.Context::addTileObject "addTileObject(center, size, rotation, subdiv, color)"</li><li>\ref pyhelios.Context.Context::addTileObject "addTileObject(center, size, rotation, subdiv, texturefile)"</li><li>\ref pyhelios.Context.Context::addTileObject "addTileObject(center, size, rotation, subdiv, texturefile, texture_repeat)"</li></ul> | ![](images/primitives/Tile.png) |
| Adaptive Tile | Patch subdivided by a quadtree into sub-patches that are fine near a target point and progressively coarser away from it. See [Adaptive Tile Objects](#AdaptiveTileObjects). | <ul><li>\ref pyhelios.Context.Context::addAdaptiveTileObject "addAdaptiveTileObject(center, size, rotation, refinement)"</li><li>\ref pyhelios.Context.Context::addAdaptiveTileObject "addAdaptiveTileObject(center, size, rotation, refinement, color)"</li><li>\ref pyhelios.Context.Context::addAdaptiveTileObject "addAdaptiveTileObject(center, size, rotation, refinement, texturefile)"</li><li>\ref pyhelios.Context.Context::addAdaptiveTileObject "addAdaptiveTileObject(center, size, rotation, refinement, texturefile, texture_repeat)"</li></ul> | |
...
#### Adaptive Tile Objects {#AdaptiveTileObjects}

A ground plane is normally built as a tile object, which subdivides it into a uniform grid. That forces an awkward tradeoff whenever the scene contains an object of interest sitting on a large domain: resolving the shadows it casts requires centimeter-scale sub-patches, while the rest of the domain only needs to occlude the horizon. Running the fine resolution all the way out wastes millions of sub-patches, and nesting several tiles of differing resolution by hand is tedious.

An adaptive tile object solves this in a single object. Sub-patches are generated by recursive quadtree subdivision, so they are finest at a target point and grow progressively coarser with distance from it. The sub-patches exactly partition the tile, with no gaps and no overlaps.

```python
from pyhelios import Context
from pyhelios.types import AdaptiveTileRefinement, vec2, vec3

context = Context()

refinement = AdaptiveTileRefinement(
    target=vec2(0, 0),           # tile-local, relative to the tile center
    subpatch_size_min=0.02,      # 2 cm beneath the plant
    subpatch_size_max=2.0,       # 2 m out at the domain edge
    transition_exponent=0.35,
)

ground = context.addAdaptiveTileObject(
    center=vec3(0, 0, 0), size=vec2(50, 50), refinement=refinement
)
```

##### Controlling the refinement

The requested size range determines a grid of coarsest-level cells spanning the tile and the number of times such a cell may be subdivided. The achieved sizes are therefore close to, but not exactly equal to, those requested — typically within about 20% at each end. Query what was actually achieved with `getAdaptiveTileObjectSubpatchSizeRange()`, which returns a `vec2` holding the achieved minimum in `x` and the achieved maximum in `y`.

`transition_exponent` controls how rapidly sub-patch size grows with distance from the target, and it is by far the most consequential parameter for the resulting sub-patch count:

| transition_exponent | Effect | Sub-patches on a 50 m tile refined from 2 m to 2 cm |
|---------------------|--------|-----------------------------------------------------|
| 0.25 | Fine resolution concentrated tightly around the target | roughly 9 thousand |
| 0.35 (default) | Suits a typical ground plane | roughly 21 thousand |
| 0.5 | Fine resolution held further out from the target | roughly 61 thousand |
| 1 | Refinement level falls linearly with radius | roughly 471 thousand |
| 2 | Fine resolution spread broadly across the tile | roughly 2 million |

For reference, a uniform tile at the same 2 cm resolution across the whole 50 m domain would need 6.25 million sub-patches. Since the count varies by more than two orders of magnitude, check the cost before building any geometry:

```python
count = context.predictAdaptiveTileObjectSubpatchCount(vec2(50, 50), refinement)
```

A refinement that would generate more than 2,000,000 sub-patches is rejected with an error rather than exhausting memory, by both `predictAdaptiveTileObjectSubpatchCount()` and `addAdaptiveTileObject()`.

##### Querying an adaptive tile

Adaptive tiles are queried through the Context with a dedicated set of methods: `getAdaptiveTileObjectCenter()`, `getAdaptiveTileObjectSize()`, `getAdaptiveTileObjectNormal()`, `getAdaptiveTileObjectVertices()`, `getAdaptiveTileObjectRefinement()`, `getAdaptiveTileObjectBaseSubdivisionCount()`, `getAdaptiveTileObjectMaxRefinementLevel()`, `getAdaptiveTileObjectSubpatchSizeRange()` and `getAdaptiveTileObjectTextureRepeat()`. `getObjectType()` returns `7` for an adaptive tile.

##### Differences from uniform tile objects

- Sub-patches are ordered by quadtree traversal rather than row-major, so they cannot be indexed by grid position.
- There is no subdivision count or area ratio. `getTileObjectAreaRatio()` and `setTileObjectSubdivisionCount()` ignore adaptive tiles with a warning, so sweeping them over every object in a scene will not overwrite an adaptive layout with a uniform grid.
- A texture repeat count is applied exactly: the base grid is snapped to a whole number of repeats. A uniform tile instead reduces the repeat count until it divides the subdivision count, which is why it has both `getTileObjectTextureRepeat()` (requested) and `getTileObjectEffectiveTextureRepeat()` (applied), while an adaptive tile has only the one.
- Plug-ins that derive a characteristic length from the parent object, such as the boundary-layer conductance model, use the dimension of the whole tile and not that of an individual sub-patch.
