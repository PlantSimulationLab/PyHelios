...
===============================================================================
    ...
    pytest tests/test_solarposition_values.py -v
...
class TestDocumentedUnitContract:
    """The sun angle getters return RADIANS, and these tests pin that.

    This is the convention everywhere else, so the getters are consistent with
    the rest of the library:

      - helios-core, plugins/solarposition/include/SolarPosition.h lines 51-58:
            "Get the current sun elevation angle in radians"
            (likewise zenith and azimuth)
      - PyHelios Context.rotatePrimitive/rotateObject: angles in radians
      - PyHelios RadiationModel camera rotation: Euler angles in radians
      - SolarPosition.getSunDirectionSpherical(), in this very class, returns
        elevation and azimuth in radians
      - SolarPosition.setSunDirection()'s own docstring example round-trips
        through math.degrees(solar.getSunElevation()) to print 45.0, which is
        only correct because the getter returns radians

    Degrees are used only where a genuinely different quantity is meant:
    latitude/longitude inputs, camera field-of-view, mesh crease angles.

    HISTORY: until v0.1.31 the docstrings on these three getters claimed
    degrees. Only the documentation was wrong — no behavior changed when it was
    corrected. These two tests were originally written as xfail(strict=True)
    against the degrees claim; they now assert the radians contract directly,
    so a future conversion inside the wrapper fails loudly here.

    Callers wanting degrees convert at the call site with math.degrees().
    """

    def test_elevation_is_returned_in_radians(self):
        """Boulder reads ~0.5147 rad, which is the ~29.49 deg reference value."""
        context, solar = _solar(BOULDER_DATE, BOULDER_TIME, BOULDER)
        try:
            elevation_rad = solar.getSunElevation()

            # Asserted in radians, against the radian form of the same
            # reference constant the degrees-based test above uses. The
            # tolerance is the selfTest allowance of +/- 10 deg, converted.
            assert elevation_rad == pytest.approx(
                math.radians(BOULDER_ELEVATION_DEG), abs=math.radians(10.0))

            # A value this small cannot be degrees: the sun is well up at
            # 10:30, so a degrees-returning implementation would read ~29.5.
            # This is what actually fails if the wrapper is ever "fixed" to
            # convert, and it does not depend on the tolerance above.
            assert elevation_rad < math.pi / 2

            # And converting gives back the documented reference value.
            assert math.degrees(elevation_rad) == pytest.approx(
                BOULDER_ELEVATION_DEG, abs=10.0)
        finally:
            solar.__exit__(None, None, None)
            context.__exit__(None, None, None)

    def test_elevation_plus_zenith_is_pi_over_two(self):
        """The complementary-angle identity, in radians.

        Independent of any reference value: elevation + zenith is a right
        angle by definition, so this reads pi/2 in radians and 90 in degrees.
        Whichever number comes back names the unit unambiguously.
        """
        context, solar = _solar((2023, 6, 21), (12, 0), dict(
            utc_offset=0, latitude=0.0, longitude=0.0))
        try:
            ...
            assert total == pytest.approx(math.pi / 2, abs=1e-4)

            # Stated the other way round, so the failure message says which
            # unit was actually returned rather than just printing a float.
            assert total != pytest.approx(90.0, abs=0.01), (
                "elevation + zenith returned 90, meaning the getters now "
                "return degrees; see this class's docstring before changing it"
            )
        finally:
            solar.__exit__(None, None, None)
            context.__exit__(None, None, None)
